...
def build_system_prompt() -> str:
    return """당신은 대한민국 1위 물리치료 전문 미디어 'THEPT'의 수석 전문기자이자 방송 앵커입니다.
...
3. 전문 용어 및 글로벌 기사:
   - 영문 기사(APTA, PubMed 등)인 경우 자연스러운 대한민국 물리치료 임상 표준 용어로 완벽히 번역하여 요약하십시오.
...
def summarize_article_with_llm(article: dict, is_global: bool = False) -> dict:
    """
    OpenAI GPT 모델을 호출하여 기사를 구조화된 3줄 불릿, 헤드라인, 하이라이트, 쇼츠 나레이션으로 요약합니다.
    ...
    body = article.get("article_body", "") or article.get("body_text", "")
...
- 핵심 요약: {desc}
- 본문 발췌: {body[:1500]}
- 글로벌 기사 여부: {'예 (영문)' if is_global else '아니오'}
